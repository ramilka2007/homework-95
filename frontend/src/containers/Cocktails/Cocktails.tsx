...
import {selectCocktails, selectUnpublishedCocktails} from "../../features/cocktails/cocktailsSlice.ts";
                        ...
                        <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 gap-5">
                            <>
                                {cocktails.map((cocktail) => (
                                    <>
                                        {user._id === cocktail.user._id || user.role === 'admin' ? (
                                            <>
                                                <CocktailItem key={cocktail._id} cocktail={cocktail} cocktailDelete={cocktailDeleter} cocktailPublish={togglePublishCocktail}/>
                                            </>
                                        ) : null}
                                    </>
                                ))}
